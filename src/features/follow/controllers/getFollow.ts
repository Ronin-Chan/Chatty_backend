...
export class GetFollow{
  public async getFollower(req: Request, res: Response): Promise<void>{
    ...
    const follower: IFollowData[] = cachedFollower.length > 0 ? cachedFollower : await followService.getFollowerFromDB(new mongoose.Types.ObjectId(req.currentUser?.userId ));
  ...
  public async getFollowee(req: Request, res: Response): Promise<void>{
    ...
    const followee: IFollowData[] = cachedFollowee.length > 0 ? cachedFollowee : await followService.getFolloweeFromDB(new mongoose.Types.ObjectId(req.currentUser?.userId ));
