...
import { authMock, authMockRequest, authMockResponse, nextFn } from '@root/mocks/auth.mock';
...
describe('Password', () => {
  ...
  describe('create', () => {
    it('should throw an error if email is invalid', () => {
      ...
      Password.prototype.create(req, res, nextFn).catch((error: CustomError) => {
    ...
    it('should send correct json response', async () => {
      ...
      await Password.prototype.create(req, res, nextFn);
  ...
  describe('update', () => {
    it('should throw an error if password is empty', () => {
      ...
      const res: Response = authMockResponse();
      Password.prototype.update(req, res, nextFn).catch((error: CustomError) => {
        expect(error.statusCode).toEqual(400);
    ...
    it('should send correct json response', async () => {
      ...
      jest.spyOn(emailQueue, 'addEmailJob');
      await Password.prototype.update(req, res, nextFn);
      expect(emailQueue.addEmailJob).toHaveBeenCalled();
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({
